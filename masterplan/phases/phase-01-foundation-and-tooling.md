...
- [x] **01.9.4** Link the source of truth — README links back to `thunder-tv`'s `.plans/` and `masterplan/`, where the plan and this tracker live.
...
- [x] **01.9.8** Add an issue-free tracker note — README states plainly that `masterplan/phases/` (in `thunder-tv`) is the tracker, not GitHub Issues.
